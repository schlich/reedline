use modalkit::editing::action::{Action, EditAction, EditorAction, EditResult, Editable};
use modalkit::editing::store::Store;
use modalkit::widgets::textbox::{TextBoxState, TextBox};
use std::io::stdout;
...
mod tests {
    ...
    #[test]
    fn insert_mode_cursor_position() {
        let mut stdout = stdout();
        let backend = CrosstermBackend::new(stdout);
        let mut terminal = Terminal::new(backend)?;
        let mut store = Store::default();
        let mut tbox = TextBoxState::new(store.load_buffer(String::from("j")));
        terminal.draw(|f| f.render_stateful_widget(TextBox::new(), f.size(), &mut tbox));
        tbox.editor_command( &Action::Editor(EditAction::Delete), &ctx, store);
        Result::Ok(())
